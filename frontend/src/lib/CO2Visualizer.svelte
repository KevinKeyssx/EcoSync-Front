...
<div class="glass-effect rounded-2xl p-6 overflow-hidden relative">
  ...
  <h2 class="text-2xl font-bold text-emerald-400 mb-6 relative z-10">
    Environmental Impact
  </h2>

  <div class="grid grid-cols-1 md:grid-cols-3 gap-4 relative z-10">
    {#each equivalencies as eq}
      <div class="bg-white/5 rounded-xl p-4 border border-emerald-400/20 hover:border-emerald-400/40 transition-all duration-300">
        <div class="flex items-center gap-3 mb-3">
          ...
          <h3 class="text-sm text-emerald-400/70">{eq.name}</h3>
        </div>
  ...
  <div class="mt-6 relative z-10">
    <div class="flex items-center justify-between text-sm mb-2">
      <span class="text-emerald-400/70">Carbon Footprint Reduction</span>
      <span class="text-emerald-400 font-semibold">{(co2SavedGrams / 1000).toFixed(3)} kg CO₂</span>
    </div>
