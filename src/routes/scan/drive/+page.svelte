<script lang="ts">
  ...
  async function deleteSelected() {
    ...
    try {
      ...
      alert(`✅ Enviados a la papelera: ${data.deleted}\n❌ Fallidos: ${data.failed}`);
    } catch (e) {
      alert('Error: ' + e);
    }
  }
        ...
        {#if drvState === 'disconnected'}
            <div class="text-center py-12 px-4" transition:fade>
                <div class="w-24 h-24 mx-auto bg-blue-500/10 border border-blue-500/20 rounded-full flex items-center justify-center mb-6 glow-effect shadow-[0_0_30px_rgba(59,130,246,0.3)]">
                    <svg class="w-12 h-12 text-blue-400" fill="currentColor" viewBox="0 0 24 24"><path d="M7 21a2 2 0 01-1.73-1.002l-3-5.2A2 2 0 012.27 14h6.002L15.27 21H7zM16.997 21l3-5.196L14.496 6.275 8.494 6.273l5.503 9.53zM15.498 5.4H8.498A2.003 2.003 0 006.77 6.4h-.001L3.77 11.603a1.996 1.996 0 000 1.996L6.77 18.8z" /></svg>
                </div>
                <h2 class="text-3xl font-bold text-white mb-4">Conecta tu Google Drive</h2>
            ...
            <div transition:fade>
                <div class="flex flex-col md:flex-row items-center justify-between mb-8 pb-8 border-b border-white/10 gap-6">
                    <div>
                        <h2 class="text-2xl font-bold text-white mb-1 flex items-center gap-3">
                            ...
                            <span class="px-3 py-1 bg-blue-500/20 text-blue-400 text-sm rounded-lg border border-blue-500/30">Completado</span>
                        </h2>
                        <p class="text-blue-400/70">
                            Se analizaron <strong class="text-white">{scanStats.totalFiles} archivos</strong> en tu cuenta.
                        </p>
                    </div>
                    
                    <div class="flex gap-3 w-full md:w-auto">
                        <button onclick={deleteAllWaste} disabled={results.filter(r=>r.isWaste).length === 0} class="w-full md:w-auto px-6 py-3 bg-red-500/10 hover:bg-red-500/20 text-red-500 font-bold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed border border-red-500/30">
                ...
                {#if results.length > 0}
                    <div class="bg-white/5 rounded-2xl border border-white/10 overflow-hidden mb-8">
                        <div class="grid grid-cols-12 gap-4 px-6 py-4 border-b border-white/10 bg-white/5 font-semibold text-sm text-blue-400/70">
                            <div class="col-span-1 flex items-center">
                                <input type="checkbox" checked={allSelected} onchange={toggleSelectAll} class="w-4 h-4 rounded border-white/20 bg-white/5 text-blue-500 focus:ring-blue-500 focus:ring-offset-slate-900 cursor-pointer" />
                            </div>
                        ...
                        <div class="divide-y divide-white/10 max-h-[500px] overflow-y-auto">
                            {#each results as result (result.id)}
                                <label class="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-white/5 transition-colors cursor-pointer group {result.isWaste ? 'border-l-2 border-l-red-500/50' : 'border-l-2 border-l-blue-500/30'}">
                                    <div class="col-span-1">
                                        {#if result.isWaste}
                                            <input type="checkbox" bind:checked={result.selected} class="w-4 h-4 rounded border-white/20 bg-white/5 text-blue-500 focus:ring-blue-500 focus:ring-offset-slate-900 cursor-pointer" />
                                        {:else}
                                            <div class="w-4 h-4"></div>
                                        {/if}
                                    </div>
                                    <div class="col-span-5 font-medium text-white flex items-center gap-2 truncate">
                                    ...
                                    <div class="col-span-3 text-sm text-blue-400/70">
                                        <span class="px-2 py-1 rounded-md text-xs inline-block capitalize mb-1 {result.isWaste ? 'bg-red-500/10 border border-red-500/30 text-red-400' : 'bg-blue-500/10 border border-blue-500/30 text-blue-400'}">{result.type}</span>
                                        {#if result.isWaste && result.wasteReason}
                                            <p class="text-[10px] text-white/40 leading-tight">{result.wasteReason}</p>
                                        {/if}
                                    </div>
                ...
                {:else}
                    <div class="text-center py-16 bg-white/5 rounded-2xl border border-dashed border-white/20">
                        <div class="w-16 h-16 mx-auto bg-blue-500/20 rounded-full flex items-center justify-center mb-4 text-blue-400 glow-effect">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                        </div>
                        <h3 class="text-xl font-bold text-white mb-2">¡Tu Drive está optimizado!</h3>
