<script lang="ts">
    ...
    async function deleteSelected( confirmed: boolean | Event = false ) {
        ...
        if ( failed > 0 ) {
            ...
            toast.success( `✅ Eliminados: ${ deleted }\n❌ Fallidos: ${ failed }` );
        }
    }
    ...
    <div class="glass-effect rounded-2xl p-6 md:p-10 relative overflow-hidden">
        ...
        {:else if ghState === 'connecting' || ghState === 'scanning'}
            <div class="text-center py-20" transition:fade>
                ...
                <p class="text-emerald-700/70 dark:text-emerald-400/70">Consultando la API de GitHub en tiempo real.</p>
            </div>
            ...
            <div transition:fade>
                <div class="flex flex-col md:flex-row items-center justify-between mb-8 pb-8 border-b border-slate-900/10 dark:border-white/10 gap-6">
                    <div>
                        <h2 class="text-2xl font-bold text-slate-900 dark:text-white mb-1 flex items-center gap-3">
                            ...
                            <span class="px-3 py-1 bg-emerald-700/20 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-sm rounded-lg border border-emerald-600/30 dark:border-emerald-500/30">Completado</span>
                        </h2>
                        <p class="text-emerald-700/70 dark:text-emerald-400/70">
                            Se analizaron <strong class="text-slate-900 dark:text-white">{scanStats.totalRepos} repositorios</strong> en tu cuenta de GitHub.
                        </p>
                    </div>
                    
                    <div class="flex gap-3 w-full md:w-auto">
                        <button onclick={deleteAll} disabled={results.length === 0} class="w-full md:w-auto px-6 py-3 bg-red-700/10 dark:bg-red-500/10 hover:bg-red-700/20 dark:hover:bg-red-500/20 text-red-700 dark:text-red-500 font-bold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed border border-red-700/30 dark:border-red-500/30">
                            Eliminar Todo (Auto)
                        </button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    <div class="bg-slate-900/5 dark:bg-white/5 rounded-xl p-5 border border-slate-900/10 dark:border-white/10 text-center">
                        ...
                        <p class="text-2xl font-bold text-slate-900 dark:text-white">{scanStats.totalRepos}</p>
                        <p class="text-xs text-emerald-700/70 dark:text-emerald-400/70 mt-1">Repos Totales</p>
                    </div>
                    <div class="bg-slate-900/5 dark:bg-white/5 rounded-xl p-5 border border-slate-900/10 dark:border-white/10 text-center">
                        ...
                        <p class="text-2xl font-bold text-yellow-700 dark:text-yellow-400">{scanStats.inactiveRepos}</p>
                        <p class="text-xs text-emerald-700/70 dark:text-emerald-400/70 mt-1">Obsoletos</p>
                    </div>
                    <div class="bg-slate-900/5 dark:bg-white/5 rounded-xl p-5 border border-slate-900/10 dark:border-white/10 text-center">
                        <div class="w-10 h-10 mx-auto rounded-lg bg-blue-700/20 dark:bg-blue-500/20 flex items-center justify-center mb-3">
                            <svg class="w-5 h-5 text-blue-700 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2 1 3 3 3h10c2 0 3-1 3-3V7c0-2-1-3-3-3H7C5 4 4 5 4 7z"></path></svg>
                        </div>
                        <p class="text-2xl font-bold text-blue-700 dark:text-blue-400">{scanStats.totalSizeGB >= 1 ? scanStats.totalSizeGB.toFixed(2) + ' GB' : (scanStats.totalSizeGB * 1024).toFixed(1) + ' MB'}</p>
                        <p class="text-xs text-emerald-700/70 dark:text-emerald-400/70 mt-1">Almacenamiento</p>
                    </div>
                    <div class="bg-slate-900/5 dark:bg-white/5 rounded-xl p-5 border border-emerald-600/20 dark:border-emerald-500/20 text-center relative overflow-hidden">
                        <div class="absolute inset-0 bg-emerald-700/5 dark:bg-emerald-500/5"></div>
                        <div class="relative">
                            <div class="w-10 h-10 mx-auto rounded-lg bg-emerald-700/20 dark:bg-emerald-500/20 flex items-center justify-center mb-3">
                            ...
                            <p class="text-xs text-emerald-700/70 dark:text-emerald-400/70 mt-1">CO₂ al año</p>
                        </div>
                    </div>
                </div>

                <!-- Equivalencias del Mundo Real -->
                <div class="bg-emerald-700/5 dark:bg-emerald-500/5 border border-emerald-600/20 dark:border-emerald-500/20 rounded-xl p-5 mb-8">
                    <h3 class="text-sm font-semibold text-emerald-700 dark:text-emerald-400 mb-3 flex items-center gap-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-emerald-700/60 dark:text-emerald-400/60 text-xs">de bombilla LED encendida</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="text-xl">🚗</span>
                            <div>
                                ...
                                <p class="text-emerald-700/60 dark:text-emerald-400/60 text-xs">recorridos en automóvil</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="text-xl">🌳</span>
                            <div>
                                ...
                                <p class="text-emerald-700/60 dark:text-emerald-400/60 text-xs">necesarios para absorberlo al año</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {#if results.length > 0}
                    <div class="bg-slate-900/5 dark:bg-white/5 rounded-2xl border border-slate-900/10 dark:border-white/10 overflow-hidden mb-8">
                        <div class="grid grid-cols-12 gap-4 px-6 py-4 border-b border-slate-900/10 dark:border-white/10 bg-slate-900/5 dark:bg-white/5 font-semibold text-sm text-emerald-700/70 dark:text-emerald-400/70">
                            <div class="col-span-1 flex items-center">
                                <input type="checkbox" checked={allSelected} onchange={toggleSelectAll} class="w-4 h-4 rounded border-slate-900/20 dark:border-white/20 bg-slate-900/5 dark:bg-white/5 text-emerald-600 dark:text-emerald-500 focus:ring-emerald-500 focus:ring-offset-slate-900 cursor-pointer" />
                            </div>
                        ...
                        <div class="divide-y divide-white/10">
                            {#each results as result (result.id)}
                                <label class="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-slate-900/5 dark:hover:bg-white/5 transition-colors cursor-pointer group {result.isWaste ? 'border-l-2 border-l-red-700/50 dark:border-l-red-500/50' : 'border-l-2 border-l-emerald-600/30 dark:border-l-emerald-500/30'}">
                                    <div class="col-span-1">
                                        <input type="checkbox" bind:checked={result.selected} class="w-4 h-4 rounded border-slate-900/20 dark:border-white/20 bg-slate-900/5 dark:bg-white/5 text-emerald-600 dark:text-emerald-500 focus:ring-emerald-500 focus:ring-offset-slate-900 cursor-pointer" />
                                    </div>
                                    <div class="col-span-5 font-medium text-slate-900 dark:text-white flex items-center gap-2">
                                        <svg class="w-5 h-5 {result.isWaste ? 'text-red-700/50 dark:text-red-400/50' : 'text-emerald-700/50 dark:text-emerald-400/50'}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"></path></svg>
                                        <a href={result.url} target="_blank" class="hover:text-emerald-700 dark:hover:text-emerald-400 transition-colors">{result.name}</a>
                                    </div>
                                    <div class="col-span-3 text-sm text-emerald-700/70 dark:text-emerald-400/70">
                                        <span class="px-2 py-1 rounded-md text-xs {result.isWaste ? 'bg-red-700/10 dark:bg-red-500/10 border border-red-700/30 dark:border-red-500/30 text-red-700 dark:text-red-400' : 'bg-emerald-700/10 dark:bg-emerald-500/10 border border-emerald-600/30 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400'}">{result.type}</span>
                                        {#if result.days_inactive > 0}
                                            <span class="ml-2 text-xs text-slate-900/40 dark:text-white/40">{result.days_inactive}d</span>
                                        {/if}
                                    </div>
                ...
                {:else}
                    <div class="text-center py-16 bg-slate-900/5 dark:bg-white/5 rounded-2xl border border-dashed border-slate-900/20 dark:border-white/20">
                        <div class="w-16 h-16 mx-auto bg-emerald-700/20 dark:bg-emerald-500/20 rounded-full flex items-center justify-center mb-4 text-emerald-700 dark:text-emerald-400 glow-effect">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                        </div>
                        <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-2">¡Todo limpio!</h3>
                        <p class="text-emerald-700/70 dark:text-emerald-400/70">No se encontraron repositorios o ramas residuales. Tu GitHub está optimizado.</p>
                    </div>
                {/if}
            </div>
        {/if}
