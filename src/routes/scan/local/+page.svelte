<script lang="ts">
  ...
  async function loadDirectories() {
    ...
    try {
      ...
      console.error( e );
      toast.error( 'Error cargando directorios locales.' );
    }
  }
  ...
  async function startScan(fullScan: boolean) {
    ...
    try {
      ...
      console.error( e );
      toast.error( 'Error en el escaneo local' );
      lState = 'selecting';
    }
  }
  ...
  async function deleteSelected() {
    ...
    try {
      ...
      if ( data.failed > 0 ) {
        toast.warning( `✅ Procesados con éxito: ${ data.deleted }\n❌ Fallidos/Omitidos: ${ data.failed }` );
      } else {
        toast.success( `✅ Procesados con éxito: ${ data.deleted }\n❌ Fallidos/Omitidos: ${ data.failed }` );
      }
    } catch ( e ) {
    ...
    <div class="glass-effect rounded-2xl p-6 md:p-10 relative overflow-hidden">
        ...
        {#if lState === 'loading_dirs'}
            <div class="text-center py-20" transition:fade>
                ...
                <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-2">Preparando Escáner Local...</h3>
                <p class="text-teal-400/70">Calculando el tamaño de tus directorios principales.</p>
            </div>

        {:else if lState === 'selecting'}
            <div class="text-center pb-8 border-b border-slate-900/10 dark:border-white/10" transition:fade>
                ...
                <h2 class="text-3xl font-bold text-slate-900 dark:text-white mb-4">Escáner de Disco Duro</h2>
            ...
            <div class="py-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" transition:fade>
                {#each directories as dir}
                    <label class="bg-slate-900/5 dark:bg-white/5 border {dir.selected ? 'border-teal-500 bg-teal-500/10' : 'border-slate-900/10 dark:border-white/10'} rounded-xl p-5 cursor-pointer hover:bg-slate-900/10 dark:hover:bg-white/10 transition-colors flex flex-col items-center text-center relative overflow-hidden group">
                        <input type="checkbox" bind:checked={dir.selected} class="absolute top-4 left-4 w-4 h-4 rounded border-slate-900/20 dark:border-white/20 bg-black/50 text-teal-500 focus:ring-teal-500" />
                        ...
                        <h4 class="text-lg font-bold text-slate-900 dark:text-white">{dir.name}</h4>
                        <p class="text-xs text-teal-400/70 break-all px-2 mt-1">{dir.path}</p>
                        
                        <div class="mt-4 pt-4 border-t border-slate-900/10 dark:border-white/10 w-full flex justify-between items-center text-sm">
                            <span class="text-slate-900 dark:text-white font-mono">{dir.size_str}</span>
                            <span class="text-slate-900/40 dark:text-white/40">{dir.files_count} arch</span>
                        </div>
                    </label>
                {/each}
            </div>
            ...
            <div class="mt-4 mb-2 flex justify-center" transition:fade>
                <button onclick={() => showAdvanced = !showAdvanced} class="text-sm flex items-center gap-2 text-teal-400 hover:text-teal-300 transition-colors bg-slate-900/5 dark:bg-white/5 hover:bg-slate-900/10 dark:hover:bg-white/10 px-4 py-2 rounded-lg border border-slate-900/10 dark:border-white/10">
            ...
            {#if showAdvanced}
                <div class="bg-black/30 w-full max-w-3xl mx-auto rounded-xl p-5 border border-slate-900/10 dark:border-white/10 mb-6 grid grid-cols-1 md:grid-cols-2 gap-6" transition:fade>
                    <div class="space-y-4">
                        <h4 class="text-slate-900 dark:text-white font-semibold flex items-center gap-2 text-sm">
                        ...
                        <div>
                            ...
                            <input type="number" bind:value={minSizeMB} min="1" class="w-full bg-black/50 border border-slate-900/10 dark:border-white/10 rounded-lg px-3 py-2 text-slate-900 dark:text-white focus:outline-none focus:border-teal-500 text-sm" />
                        </div>
                        
                        <div>
                            <label class="block text-xs text-white/50 mb-1">Antigüedad mínima de inactividad (Meses)</label>
                            <input type="number" bind:value={minMonthsOld} min="0" class="w-full bg-black/50 border border-slate-900/10 dark:border-white/10 rounded-lg px-3 py-2 text-slate-900 dark:text-white focus:outline-none focus:border-teal-500 text-sm" />
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <input type="checkbox" bind:checked={includeTemps} class="w-4 h-4 rounded border-slate-900/20 dark:border-white/20 bg-black/50 text-teal-500 focus:ring-teal-500" />
                            <span class="text-sm text-slate-900/70 dark:text-white/70 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">Archivos temporales (Logs, .tmp, .bak)</span>
                        </label>
                        
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <input type="checkbox" bind:checked={includeInstallers} class="w-4 h-4 rounded border-slate-900/20 dark:border-white/20 bg-black/50 text-teal-500 focus:ring-teal-500" />
                            <span class="text-sm text-slate-900/70 dark:text-white/70 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">Instaladores y binarios obsoletos (.exe, .msi)</span>
                        </label>

                        <label class="flex items-center gap-3 cursor-pointer group">
                            <input type="checkbox" bind:checked={includeCache} class="w-4 h-4 rounded border-slate-900/20 dark:border-white/20 bg-black/50 text-teal-500 focus:ring-teal-500" />
                            <span class="text-sm text-slate-900/70 dark:text-white/70 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">Cachés ocultas de aplicaciones perdidas</span>
                        </label>
                    </div>
                </div>
            {/if}
            ...
            <div class="flex flex-col sm:flex-row justify-center gap-4 pt-4">
                <button onclick={() => startScan(false)} disabled={selectedDirsCount === 0} class="px-8 py-4 bg-slate-900/10 dark:bg-white/10 hover:bg-slate-900/20 dark:hover:bg-white/20 text-slate-900 dark:text-white rounded-xl font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        {:else if lState === 'scanning'}
            <div class="text-center py-20" transition:fade>
                ...
                <h3 class="text-2xl font-bold text-slate-900 dark:text-white mb-2">Escaneo profundo en curso...</h3>
        ...
        {:else if lState === 'results'}
            <div transition:fade>
                <div class="flex flex-col md:flex-row items-center justify-between mb-8 pb-8 border-b border-slate-900/10 dark:border-white/10 gap-6">
                    <div>
                        ...
                        <p class="text-teal-400/70">
                            Se analizaron <strong class="text-slate-900 dark:text-white">{scanStats.totalFiles} archivos</strong> físicos en tu disco duro.
                        </p>
                    </div>
                    
                    <button onclick={() => loadDirectories()} class="px-4 py-2 bg-slate-900/5 dark:bg-white/5 hover:bg-slate-900/10 dark:hover:bg-white/10 border border-slate-900/10 dark:border-white/10 rounded-lg text-sm text-slate-900 dark:text-white">
                        Volver a escanear
                    </button>
                </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                    ...
                    <div class="bg-slate-900/5 dark:bg-white/5 rounded-xl p-5 border border-slate-900/10 dark:border-white/10 text-center">
                        <div class="w-10 h-10 mx-auto rounded-lg bg-red-700/20 dark:bg-red-500/20 flex items-center justify-center mb-3">
                            <svg class="w-5 h-5 text-red-700 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2 1 3 3 3h10c2 0 3-1 3-3V7c0-2-1-3-3-3H7C5 4 4 5 4 7z"></path></svg>
                        </div>
                        <p class="text-2xl font-bold text-red-700 dark:text-red-400">{scanStats.totalSizeGB >= 1 ? scanStats.totalSizeGB.toFixed(2) + ' GB' : (scanStats.totalSizeGB * 1024).toFixed(1) + ' MB'}</p>
                ...
                {#if results.length > 0}
                    <div class="bg-slate-900/5 dark:bg-white/5 rounded-2xl border border-slate-900/10 dark:border-white/10 overflow-hidden mb-6">
                        <div class="grid grid-cols-12 gap-4 px-6 py-4 border-b border-slate-900/10 dark:border-white/10 bg-slate-900/5 dark:bg-white/5 font-semibold text-sm text-teal-400/70">
                            <div class="col-span-1 flex items-center">
                                <input type="checkbox" checked={allResultsSelected} onchange={toggleSelectAllResults} class="w-4 h-4 rounded border-slate-900/20 dark:border-white/20 bg-slate-900/5 dark:bg-white/5 text-teal-500 focus:ring-teal-500" />
                            </div>
                        ...
                        <div class="divide-y divide-white/10 max-h-[500px] overflow-y-auto">
                            {#each results as result (result.id)}
                                <label class="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-slate-900/5 dark:hover:bg-white/5 transition-colors cursor-pointer group border-l-2 border-l-red-700/50 dark:border-l-red-500/50">
                                    <div class="col-span-1">
                                        <input type="checkbox" bind:checked={result.selected} class="w-4 h-4 rounded border-slate-900/20 dark:border-white/20 bg-slate-900/5 dark:bg-white/5 text-teal-500 focus:ring-teal-500" />
                                    </div>
                                    <div class="col-span-5 flex-col min-w-0 pr-2">
                                        <p class="font-medium text-slate-900 dark:text-white truncate" title={result.name}>{result.name}</p>
                                        <p class="text-[10px] text-slate-900/40 dark:text-white/40 truncate mt-1" title={result.path}>{result.path}</p>
                                    </div>
                                    <div class="col-span-3 text-sm text-teal-400/70">
                                        <span class="px-2 py-1 rounded-md text-xs inline-block capitalize mb-1 bg-red-700/10 dark:bg-red-500/10 border border-red-700/30 dark:border-red-500/30 text-red-700 dark:text-red-400">{result.waste_category}</span>
                                        {#if result.waste_reason}
                                            <p class="text-[10px] text-slate-900/40 dark:text-white/40 leading-tight">{result.waste_reason}</p>
                                        {/if}
                                    </div>
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center bg-black/20 rounded-xl p-4 border border-white/5 gap-4">
                        <div class="flex items-center gap-4 bg-slate-900/5 dark:bg-white/5 px-4 py-3 rounded-lg border border-slate-900/10 dark:border-white/10">
                            <span class="text-sm font-medium text-slate-900/70 dark:text-white/70">Método de borrado:</span>
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" bind:checked={deletePermanently} class="sr-only peer">
                                <div class="w-11 h-6 bg-slate-900/10 dark:bg-white/10 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-red-500"></div>
                                <span class="ml-3 text-sm font-bold {deletePermanently ? 'text-red-700 dark:text-red-400' : 'text-teal-400'}">
                                    {deletePermanently ? 'Destructivo (Bypass Papelera)' : 'Mover a Papelera de Reciclaje'}
                                </span>
                            </label>
                        </div>

                        <button onclick={deleteSelected} disabled={resultSelectedCount === 0} class="w-full md:w-auto px-8 py-4 {deletePermanently ? 'bg-red-500 hover:bg-red-600' : 'bg-teal-500 hover:bg-teal-600'} text-slate-900 dark:text-white font-bold rounded-xl active:scale-95 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-xl">
                ...
                {:else}
                    <div class="text-center py-16 bg-slate-900/5 dark:bg-white/5 rounded-2xl border border-dashed border-slate-900/20 dark:border-white/20">
                        ...
                        <h3 class="text-xl font-bold text-slate-900 dark:text-white mb-2">¡Unidades Limpias!</h3>
