<script lang="ts">
  ...
  let state: OsintState = $state('idle');
  let username = $state('');
  ...
  async function startOsintScan() {
    ...
    try {
      ...
      console.error(e);
      alert('Hubo un error contactando al servidor. Asegúrate de que el backend esté ejecutándose.');
      state = 'idle';
    }
  }
    ...
    <div class="glass-effect rounded-2xl p-6 md:p-10 relative overflow-hidden">
        ...
        {#if state === 'idle'}
            <div class="text-center py-12 px-4 max-w-2xl mx-auto" transition:fade>
                ...
                <h2 class="text-3xl font-bold text-white mb-4">Cazador de Cuentas Fantasmas</h2>
                ...
                <div class="flex flex-col sm:flex-row gap-4 max-w-lg mx-auto mb-6">
                    <input 
                        ...
                        class="flex-1 bg-black/40 border border-purple-500/30 rounded-xl px-6 py-4 text-white text-lg focus:outline-none focus:border-purple-500 transition-colors shadow-inner"
                    />
                    <button 
        ...
        {:else if state === 'scanning'}
            <div class="text-center py-20" transition:fade>
                ...
                <h3 class="text-2xl font-bold text-white mb-2">Pulsando servidores web...</h3>
        ...
        {:else if state === 'results'}
            <div transition:fade>
                <div class="flex flex-col md:flex-row items-center justify-between mb-8 pb-8 border-b border-white/10 gap-6">
                    ...
                    <button onclick={() => state = 'idle'} class="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-sm text-white transition-colors">
                        Nueva Búsqueda
                    </button>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    <div class="bg-white/5 rounded-xl p-5 border border-white/10 flex items-center gap-4">
                    ...
                    <div class="bg-purple-500/5 rounded-xl p-5 border border-purple-500/20 flex items-center gap-4 relative overflow-hidden">
                        <div class="w-12 h-12 rounded-lg bg-emerald-500/20 flex flex-shrink-0 items-center justify-center relative z-10">
                            <svg class="w-6 h-6 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        </div>
                        <div class="relative z-10">
                            <p class="text-3xl font-bold text-emerald-400">{scanStats.totalCo2 >= 1000 ? (scanStats.totalCo2 / 1000).toFixed(2) + ' kg' : scanStats.totalCo2 + ' g'}</p>
                ...
                {#if results.length > 0}
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {#each results as account}
                            <div class="bg-white/5 border border-white/10 rounded-xl p-5 hover:bg-white/10 transition-colors group flex flex-col justify-between h-full">
                                <div>
                                    <div class="flex items-center justify-between mb-3">
                                        <div class="flex items-center gap-2">
                                            <span class="text-2xl">{getPlatformIcon(account.platform)}</span>
                                            <h4 class="text-lg font-bold text-white">{account.platform}</h4>
                                        </div>
                                        <span class="text-xs font-mono px-2 py-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 rounded">
                                ...
                                <div class="flex gap-2 mt-auto">
                                    <a href={account.profile_url} target="_blank" rel="noopener noreferrer" class="flex-1 px-3 py-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white/80 text-center text-sm font-medium rounded-lg transition-colors">
                                        Ver Perfil
                                    </a>
                                    <a href={account.delete_url} target="_blank" rel="noopener noreferrer" class="flex-1 px-3 py-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 text-center text-sm font-bold rounded-lg transition-colors">
                                        Ir a Borrar
                                    </a>
                                </div>
                            </div>
                ...
                {:else}
                    <div class="text-center py-16 bg-white/5 rounded-2xl border border-dashed border-white/20">
                        ...
                        <h3 class="text-xl font-bold text-white mb-2">¡Rastro digital limpio!</h3>
