    ...
    {#each tools as tool}
        <svelte:element 
            ...
            class="text-left rounded-2xl p-6 transition-all duration-300 relative overflow-hidden flex flex-col h-full focus:outline-none focus:ring-2 focus:ring-emerald-500/50
                {tool.status === 'active' 
                    ? 'glass-effect hover:bg-slate-900/10 dark:hover:bg-white/10 hover:-translate-y-1 cursor-pointer group hover:shadow-[0_0_20px_rgba(52,211,153,0.1)] block' 
                    : 'bg-slate-900/5 dark:bg-white/5 border border-white/5 opacity-50 cursor-not-allowed filter grayscale'}"
        >
            <div class="flex items-center justify-between mb-4 relative z-10 w-full">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-300
                    {tool.status === 'active' ? 'bg-emerald-700/20 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 group-hover:scale-110 glow-effect' : 'bg-slate-900/10 dark:bg-white/10 text-white/50'}">
                ...
                {#if tool.status === 'dev'}
                    <span class="px-2 py-1 rounded bg-slate-900/10 dark:bg-white/10 text-white/50 text-xs font-medium tracking-wide">
                        En Desarrollo
                    </span>
                {:else}
                    <span class="px-2 py-1 rounded bg-emerald-700/20 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border border-emerald-400/30 text-xs font-semibold tracking-wide shadow-[0_0_10px_rgba(16,185,129,0.1)] glow-effect">
                        Disponible
                    </span>
                {/if}
            </div>

            <h3 class="relative z-10 text-xl font-bold {tool.status === 'active' ? 'text-slate-900 dark:text-white' : 'text-slate-900/70 dark:text-white/70'} mb-2">{tool.name}</h3>
            <p class="relative z-10 {tool.status === 'active' ? 'text-emerald-700/70 dark:text-emerald-400/70' : 'text-slate-900/40 dark:text-white/40'} text-sm leading-relaxed">{tool.desc}</p>
        </svelte:element>
    {/each}
</div>
